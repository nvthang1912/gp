			...
			for (int i = 0; i < nodeList.getLength(); ++i) {
				...
				if (childNode.getNodeName().equals("baseDir")) {
					if (servletConf.getInitParameter("baseDir") == null) { // baseDir cua application.prop la null
						this.baseDir = childNode.getTextContent().trim(); // lay the ben ckfinder.xml, khi do phai thiet
																			// lap ben ckfinder.xml
				...
				if (childNode.getNodeName().equals("baseURL")) {
					if (servletConf.getInitParameter("baseURL") == null) {
						...
						this.baseURL = servletConf.getInitParameter("baseURL") + "/uploadmedia/";
					}
				...
				if (childNode.getNodeName().equals("licenseName")) {
					if (servletConf.getInitParameter("licenseName") == null) {
						this.licenseName = childNode.getTextContent().trim();
					} else {
						this.licenseName = servletConf.getInitParameter("licenseName");
					}
				}
				if (childNode.getNodeName().equals("licenseKey")) {
					if (servletConf.getInitParameter("licenseKey") == null) {
						this.licenseKey = childNode.getTextContent().trim();
					} else {
						this.licenseKey = servletConf.getInitParameter("licenseKey");
					}
				}
				String value;
