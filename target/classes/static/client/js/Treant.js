        ...
        if (this.text) {
            for (var key in this.text) {
                ...
                if (key.startsWith("data-")) {
                	...
                	if (this.text[key] != null && (key == 'name' || key == 'address')) { 
                	...
                	if (key == 'id') {
                		str += "ID:  " + this.text[key] + '<br>';
                	}
                	if (key == 'name') {
                		str += "Tên:  " + this.text[key] + '<br>';
                	}
                	if (key == 'phone') {
                		str += "Điện thoại:  " + this.text[key] + '<br>';
                	}
                	if (key == 'address') {
                		str += "Địa chỉ:  " + this.text[key] + '<br>';
                	}
                	if (key == 'deathday') {
                		str += "Ngày mất:  " + this.text[key] + '<br>';
                	}
                	if (key == 'parentName') {
                		str += "Tên cha:  " + this.text[key];
                	}
                }
            }
            
            var id = this.text['id'];
            var d = '<div class="modal fade" id="info-dialog-' + this.text['id'] + '" tabindex="-1" role="dialog" aria-hidden="true">'
					+ '<div class="modal-dialog"><div class="modal-content">' + 
					'<div class="modal-body"> ' + str + ' </div><div class="modal-footer"><button type="button" class="btn btn-danger" data-dismiss="modal">Đóng</button></div></div></div></div>';
            
            $("#dialog-info").append(d);
            node.onclick = function() { $('#info-dialog-' + id ).modal('show'); };
        }
        return node;
    };
